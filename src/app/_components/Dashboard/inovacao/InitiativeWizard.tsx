    ...
    dataToEdit.relatedTo && dataToEdit.relatedTo.length > 0
      ...
      : undefined;
        ...
        form.reset({
          ...
          type: dataToEdit.type as CreateInovationValues["type"],
...
const onFormSubmit = (values: CreateInovationValues) => {
  ...
  const onFormError = (errors: FieldErrors<CreateInovationValues>) => {
                      ...
                      <div className="grid grid-cols-1 gap-4 max-h-[350px] overflow-y-auto">
                        {SOCIO.map(({ key, label, name, description }) => (
                          <FormField
                            key={label}
                            control={form.control}
                            name={name as keyof CreateInovationValues}
                            render={({ field }) => (
                              <SocioInput
                                letter={key}
                                label={label}
...
interface DateFieldProps {
  control: Control<CreateInovationValues>;
  name: keyof CreateInovationValues;
  label: string;
  disabled?: boolean;
}

const DateField = ({ control, name, label, disabled }: DateFieldProps) => (
  <FormField
    control={control}
    name={name}
    render={({ field }) => {
...
interface SocioInputProps {
  letter: string;
  label: string;
  description: string;
  field: ControllerRenderProps<CreateInovationValues>;
  disabled?: boolean;
}

const SocioInput = ({ letter, label, description, field, disabled }: SocioInputProps) => (
