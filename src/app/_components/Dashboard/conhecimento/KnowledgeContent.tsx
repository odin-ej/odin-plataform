...
const KnowledgeContent = () => {
  ...
  const { mutate: uploadFile, isPending: isLoading } = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage =
        axios.isAxiosError(error) ? error.response?.data?.message : "Falha no upload do arquivo.";
      toast.error("Erro no Upload", { description: errorMessage || "Falha no upload do arquivo." });
    },
  });
