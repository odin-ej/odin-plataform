...
type FeedCacheData = InfiniteQueryData | InitialFeedData;
...
const PostCard = ({ post, currentUserId }: PostCardProps) => {
  ...
  const { mutate: likePost } = useMutation({
    ...
    onMutate: async () => {
      ...
      const previousFeedData = queryClient.getQueryData<InfiniteQueryData>([
        "feedPosts",
        currentUserId,
      ]);
      const previousInitialData = queryClient.getQueryData<InitialFeedData>([
        "initialFeedData",
        currentUserId,
      ]);

      // Atualização otimista
      const updatePostData = (oldData: FeedCacheData | undefined) => {
        if (!oldData) return oldData;
        const updatePages = (pages: FeedPage[]) =>
          pages.map((page) => ({
            ...page,
        ...
        } else if ("initialFeed" in oldData) {
          // Initial data query
          return {
            ...oldData,
            initialFeed: {
  ...
  const { mutate: savePost } = useMutation({
    ...
    onMutate: async () => {
      ...
      const updateCacheData = (oldData: FeedCacheData | undefined) => {
        if (!oldData) return oldData;
        ...
        else if ("initialFeed" in oldData || "pinnedPosts" in oldData) {
          return {
            ...oldData,
            pinnedPosts:
