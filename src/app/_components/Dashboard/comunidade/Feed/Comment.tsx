...
const Comment = ({ comment, postId, currentUserId }: CommentProps) => {
  ...
  const { mutate: likeComment } = useMutation({
      ...
      onMutate: async () => {
          ...
          const updateCommentData = (oldData: InfiniteCommentData | undefined) => {
  ...
  const { mutate: handleDeleteComment } = useMutation({
    ...
    onSuccess: () => {
        ...
        queryClient.setQueryData(['comments', postId], (oldData: InfiniteCommentData | undefined) => updateDataToRemoveComment(oldData, comment.id));
        if (comment.parentId) queryClient.setQueryData(['replies', comment.parentId], (oldData: InfiniteCommentData | undefined) => updateDataToRemoveComment(oldData, comment.id));
  ...
  const updateDataToRemoveComment = (oldData: InfiniteCommentData | undefined, commentIdToRemove: string) => {
      if (!oldData?.pages) return oldData;
      return {
          ...oldData,
          pages: oldData.pages.map((page: CommentPage) => ({
              ...
              items: page.items.filter((c: FullComment) => c.id !== commentIdToRemove)
                               .map((c: FullComment) => ({ // Remove das replies tbm
                                   ...
                                   replies: Array.isArray(c.replies)
                                     ? c.replies.filter((r) => r.id !== commentIdToRemove)
                                     : c.replies
                               }))
          }))
      };
  ...
  // --- Mutação para Criar Resposta ---
   const { mutate: addReply, isPending: isReplying } = useMutation({
     ...
     onSuccess: (data) => {
         ...
         queryClient.setQueryData(['replies', comment.id], (oldData: InfiniteCommentData | undefined) => {
