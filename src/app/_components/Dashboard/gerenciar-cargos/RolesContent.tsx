...
const RolesContent = ({ initialData }: RolesContentProps) => {
  ...
  const { mutate: createRole, isPending: isCreating } = useMutation({
    ...
    onError: (error: unknown) => {
      toast.error(`Erro ao criar cargo!: ${axios.isAxiosError(error) ? error.response?.data?.message : "Erro desconhecido"}`);
    },
  });
  ...
  const { mutate: updateRole, isPending: isUpdating } = useMutation({
    ...
    onError: (error: unknown) => {
      toast.error(`Erro ao atualizar cargo!: ${axios.isAxiosError(error) ? error.response?.data?.message : "Erro desconhecido"}`);
    },
  });
  ...
  const { mutate: deleteRole, isPending: isDeleting } = useMutation({
    ...
    onError: (error: unknown) => {
      toast.error(`Erro ao deletar cargo!: ${axios.isAxiosError(error) ? error.response?.data?.message : "Erro desconhecido"}`);
    },
  });

  const { mutate: saveAttributes, isPending: isSavingAttributes } = useMutation(
    {
      mutationFn: (data: { categories: { id?: string; name: string }[]; interests: { id?: string; name: string; categoryId: string }[] }) =>
     ...
     onError: (error: unknown) => {
      toast.error(`Erro ao atualizar atributos!: ${axios.isAxiosError(error) ? error.response?.data?.message : "Erro desconhecido"}`);
    },
    }
  );
