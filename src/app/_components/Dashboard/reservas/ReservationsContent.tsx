...
type CombinedReservation =
  | (ExtendedReservation & { type: "salinha" })
  | (ItemWithRelations & { type: "item" })
  | (RequestWithApplicant & { type: "eaufba" });
...
const ReservationsContent = ({
  ...
  const isGerenteConexoes = checkUserPermission(user, { allowedRoles: ['Gerente de Conexões'] });
  const canManageReservations = isDirector || isGerenteConexoes;
  ...
  const { mutate: createReservation, isPending: isCreatingReservation } =
    useMutation({
      ...
      onError: (error: AxiosError<{ message: string }>) => {
        toast.error("Houve um erro ao realizar a reserva", {
    ...
    <>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6">
        <DailyScheduleView
          rooms={rooms}
          items={reservableItems}
          reservations={allReservations as CombinedReservation[]}
        />
        <UnifiedCalendar
          events={allEvents}
