...
const UsefulLinksSection = ({
  ...
  const { mutate: addLink, isPending: isAdding } = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage =
        axios.isAxiosError(error) ? error.response?.data?.message : "Erro ao adicionar link.";
      toast.error(errorMessage || "Erro ao adicionar link.");
    },
  });
  ...
  const { mutate: updateLink, isPending: isUpdating } = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage =
        axios.isAxiosError(error) ? error.response?.data?.message : "Erro ao atualizar link.";
      toast.error(errorMessage || "Erro ao atualizar link.");
    },
  });
  ...
  const { mutate: deleteLink, isPending: isDeleting } = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage =
        axios.isAxiosError(error) ? error.response?.data?.message : "Erro ao remover link.";
      toast.error(errorMessage || "Erro ao remover link.");
    },
  });
