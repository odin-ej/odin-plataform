...
const HomeContent = ({ initialData }: { initialData: HomeContentData }) => {
  ...
  const fetchHomeData = async (): Promise<HomeContentData> => {
    ...
    const goals = strategyRes.data?.flatMap((obj: { goals: Goal[] }) => obj.goals) || [];
  ...
  const { data: eventNotifications = [] } = useQuery({
    ...
    queryFn: () => getEventNotifications(),
    enabled: !!user,
  });

  const dismissMutation = useMutation({
    mutationFn: (id: string) => dismissEventNotification(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["eventNotifications"] });
    },
  });
    ...
    <>
      ...
      {eventNotifications.length > 0 && (
        <div className="space-y-2 mb-4">
          {eventNotifications.map((en) => (
            <div
              key={en.id}
              className="flex items-center justify-between gap-3 rounded-lg border bg-yellow-950/30 border-yellow-800 px-4 py-3"
            >
              ...
              <button
                onClick={() => dismissMutation.mutate(en.id)}
                className="shrink-0 rounded-md p-1 hover:bg-yellow-800 transition-colors"
              >
