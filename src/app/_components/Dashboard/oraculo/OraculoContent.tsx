...
const OraculoContent = ({ initialData }: { initialData: OraculoPageProps }) => {
  ...
  const allVisibleItems = useMemo(() => {
    ...
    const hasDirectPermission = (item: FullOraculoFile | FullOraculoFolder) => {
        ...
        if (restrictedAreas.includes(OraculoAreas.GERAL as OraculoAreas)) return true;
  ...
  const { mutate: actionMutation, isPending: isActionLoading } = useMutation({
    mutationFn: async ({
      ...
      payload: FormData | { item: FullOraculoFile | FullOraculoFolder; name: string; restrictedToAreas?: OraculoAreas[] };
    }) => {
      if (type === "rename") {
        const renamePayload = payload as { item: FullOraculoFile | FullOraculoFolder; name: string; restrictedToAreas?: OraculoAreas[] };
        const { item, name } = renamePayload;
        const itemType = "parentId" in item ? "folders" : "files";
        return axios.patch(`${API_URL}/api/oraculo/${itemType}/${item.id}`, {
          name,
          restrictedToAreas: renamePayload.restrictedToAreas,
        });
      }
  ...
  const handleAction = (type: OraculoActionType, item?: FullOraculoFile | FullOraculoFolder) => {
    setTimeout(() => {
