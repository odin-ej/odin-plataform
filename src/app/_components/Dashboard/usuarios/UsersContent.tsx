  ...
  const memberColumns: ColumnDef<UniversalMember>[] = [
    ...
    { accessorKey: "phone", header: "Telefone" },
    {
      ...
      cell: (row) => {
        ...
        if (("roleId" in row ? (row as { roleId: string }).roleId : undefined)) {
          return (
            availableRoles.find((role) => role.id === ("roleId" in row ? (row as { roleId: string }).roleId : undefined))
              ?.name || "Outro"
          );
        }
      ...
      ? [
          {
            ...
            cell: (row: UniversalMember) => {
              ...
              const lastActive = "lastActiveAt" in row ? (row as { lastActiveAt?: Date | null }).lastActiveAt : null;
      ...
      const role = formatedRoles.find(
        (role) =>
          ...
          (type === "users"
            ...
            : (data as UserProfileValues & { roleId?: string }).roleId)
      )?.label;
        ...
        ? {
            currentRoleId:
              "currentRole" in user && user.currentRole
                ? user.currentRole.id
                : ("roleId" in user ? (user as { roleId: string }).roleId : "") || "",
          }
        : {
            roleId:
              "currentRole" in user && user.currentRole
