...
const PerfilContent = ({ initialData }: { initialData: PerfilPageData }) => {
  ...
  const { mutate: updateProfile, isPending: isUpdating } = useMutation({
    mutationFn: async (formData: MemberUpdateType | ExMemberUpdateType) => {
      const { image, roleHistory, ...restOfData } = formData as MemberUpdateType & { image?: File; roleHistory?: Record<string, unknown>[] };
      ...
      const finalRoleHistory = await Promise.all(
        (roleHistory || []).map(async (historyItem: Record<string, unknown>, index: number) => {
