...
const MyPointsContent = ({ initialData }: { initialData: MyPointsData }) => {
  ...
  const handleOpenEditModal = (
    ...
    if (type === "solicitation") {
      ...
      requestForm.reset({
        ...
        tags: sol.solicitationTags.map((t) => t.tagTemplateId),
        membersSelected: sol.membersSelected.map((m) => m.id),
      });
    } else {
  ...
  const targetColumn: ColumnDef<FullJRPointsSolicitation | FullJRPointsReport> = {
  ...
  const requestFields = useMemo((): FieldConfig<FormDataType>[] => {
    ...
    const memberSelect: FieldConfig<FormDataType> = {
      accessorKey: "membersSelected" as Path<FormDataType>,
    ...
    const fields = [
      {
        accessorKey: "description" as Path<FormDataType>,
    ...
    <>
      ...
      {itemToDelete && (
        <ModalConfirm
          ...
          onConfirm={() => deleteRequest(itemToDelete)}
