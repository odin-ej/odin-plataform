...
const getAttachments = (item: HistoryItemData): AttachmentRecord[] => {
  if (item.type === "solicitation" || item.type === "report") {
    return (item.data.attachments as unknown as AttachmentRecord[]) ?? [];
  }
  return [];
};
...
const HistoryItemDetailsModal = ({
  ...
  useEffect(() => {
    const fetchSignedUrls = async () => {
      ...
      await Promise.all(
        attachments.map(async (file: AttachmentRecord) => {
          try {
            ...
            <div className="px-6 pb-6 pt-2">
              ...
              <div className="space-y-2">
                {loadingSignedUrls ? (
                  ...
                  getAttachments(item).map((file: AttachmentRecord) => (
                    <a
