...
interface UpdateData {
  ...
  id: string;
  [key: string]: string; // Permite outros campos como 'name', 'description', etc.
}
...
export function StrategyObjectivesSection({ initialObjectives }: Props) {
  ...
  const { mutate: updateItem, isPending: isUpdating } = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage =
        axios.isAxiosError(error) ? error.response?.data?.message : "Falha ao atualizar.";
      toast.error("Erro ao atualizar", { description: errorMessage || "Falha ao atualizar." });
    },
  });
