...
const UpdateStrategyContent = ({
  ...
  const { mutate: updateStrategy, isPending: isUpdating } = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage =
        axios.isAxiosError(error) ? error.response?.data?.message : "Falha ao atualizar o valor.";
      toast.error("Erro ao atualizar", { description: errorMessage || "Falha ao atualizar o valor." });
