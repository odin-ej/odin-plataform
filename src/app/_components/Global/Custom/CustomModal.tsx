...
const CustomModal = <T extends FieldValues>({
  ...
  const [formRef, setFormRef] = useState<UseFormReturn<T> | null>(null);
  ...
  const handleFileSelect = (file: File, formInstance: UseFormReturn<T>) => {
    if (file) {
  ...
  const handleCropComplete = (croppedImageBlob: Blob) => {
    if (formRef) {
      ...
      formRef.setValue("image" as Path<T>, croppedFile as PathValue<T, Path<T>>, {
        shouldValidate: true,
        shouldDirty: true,
      });
    }
