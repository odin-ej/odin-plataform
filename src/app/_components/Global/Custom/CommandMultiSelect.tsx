...
const CommandMultiSelect = ({ form, name, label, options }: { form: UseFormReturn<any>, name: string, label: string, options: { value: string, label: string }[] }) => {
    ...
    return (
        <FormField
            ...
            render={({ field }) => {
                ...
                const selectedItems = options.filter(opt => selectedValues.has(opt.value));
                                        ...
                                        <CommandGroup>
                                            {options.map((option) => (
                                                <CommandItem
                                                    ...
                                                    className="cursor-pointer hover:!bg-white/10 hover:!text-[#f5b719] transition-colors"
                                                >
                                                    <Check
                                                        className={cn("mr-2 h-4 w-4", selectedValues.has(option.value) ? "opacity-100" : "opacity-0")}
                                                    />
                                                    {option.label}
                                                </CommandItem>
                                            ))}
