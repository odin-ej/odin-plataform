...
import { DefaultValues, FormProvider, Path, UseFormReturn, useForm } from "react-hook-form";
...
interface ExMemberFormProps<T extends z.ZodTypeAny> {
  isLoading: boolean;
  ...
  onFileSelect?: (file: File, form: UseFormReturn<z.infer<T>>) => void;
...
const ExMemberForm = <T extends z.ZodTypeAny>({
  title = "Cadastrar",
  values,
  onSubmit,
  onFileSelect,
  ...
  const defaultFormPerfilValues = {
    ...
    isExMember: "Sim",
    alumniDreamer: (values as Record<string, unknown>)?.alumniDreamer ? "Sim" : "Não",
  };
