...
const CalendarView = ({
  ...
  const [gsiClient, setGsiClient] = useState<{ requestAccessToken: () => void } | null>(null);
  ...
  useEffect(() => {
    const windowWithGoogle = window as Window & { google?: { accounts: { oauth2: { initTokenClient: (config: Record<string, unknown>) => { requestAccessToken: () => void }; revoke: (token: string, callback: () => void) => void } } } };
    if (clientId && windowWithGoogle.google && windowWithGoogle.google.accounts) {
      const client = windowWithGoogle.google.accounts.oauth2.initTokenClient({
        client_id: clientId,
  ...
  const handleLogout = () => {
    const windowWithGoogle = window as Window & { google?: { accounts: { oauth2: { revoke: (token: string, callback: () => void) => void } } } };
    if (token && windowWithGoogle.google) {
      windowWithGoogle.google.accounts.oauth2.revoke(token, () => {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        const correctedEvents = (data.items || []).map((event: CalendarEvent) => {
          let internalDate;
          if (event.start.date) {
