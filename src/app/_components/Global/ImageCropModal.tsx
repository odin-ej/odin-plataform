 ...
 */
async function cropImage(imageSrc: string, pixelCrop: Area): Promise<Blob | null> {
...
const ImageCropModal = ({ imageSrc, onClose, onCropComplete, cropShape, aspect }: ImageCropModalProps) => {
  const [crop, setCrop] = useState<Point>({ x: 0, y: 0 });
  const [zoom, setZoom] = useState(1);
  const [croppedAreaPixels, setCroppedAreaPixels] = useState<Area | null>(null);
  const [isCropping, setIsCropping] = useState(false);

  const onCropAreaChange = useCallback((_croppedArea: Area, croppedAreaPixelsValue: Area) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[999]">
      <div className="pointer-events-auto bg-[#010d26] p-6 rounded-xl w-[90%] max-w-lg relative border border-[#0126fb]">
        <h2 className="mb-5 text-white font-semibold text-xl text-center">Editar Imagem</h2>
