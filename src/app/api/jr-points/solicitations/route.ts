  ...
  try {
    ...
    if (!activeSemester) {
      return NextResponse.json(
        {
          message:
            "Nenhum semestre está ativo. Um administrador precisa iniciar um novo período.",
        },
        { status: 400 }
      );
    }
    if (!activeVersion) {
      return NextResponse.json(
        { message: "Nenhuma versão de regras do JR Points está ativa." },
        { status: 400 }
      );
    }
      ...
      if (isForEnterprise) {
        ...
        if (!membersSelected)
          return NextResponse.json(
            {
              message:
                "Não existem membros selecionados. Por favor selecione pelo menos um!",
            },
            { status: 400 }
          );
