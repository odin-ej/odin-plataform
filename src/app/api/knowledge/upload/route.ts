...
export async function POST(request: Request) {
  try {
    ...
    if (file.size > 30 * 1024 * 1024) {
      return NextResponse.json({ message: "Arquivo muito grande (máx. 30MB)." }, { status: 400 });
    }
    ...
    if (!textContent.trim()) {
      return NextResponse.json({ message: "O ficheiro está vazio ou ilegível." }, { status: 400 });
    }

    // Divide o texto em pedaços (chunks)
    const chunks = textContent.match(/.{1,1500}/gs) || [];
    if (chunks.length === 0) {
      return NextResponse.json({ message: "Não foi possível extrair conteúdo do ficheiro." }, { status: 400 });
    }
