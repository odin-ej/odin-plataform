...
export async function getPresignedUrlForAttachment(
  ...
  if (fileSize > S3_UPLOAD.MAX_ATTACHMENT_SIZE_BYTES)
  ...
  const url = await getSignedUrl(s3Client, command, { expiresIn: S3_UPLOAD.PRESIGNED_URL_EXPIRY_SECONDS });
...
export async function updateConversationTitle({
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Falha ao atualizar o título.";
    return {
      success: false,
      message,
    };
