...
import { checkUserPermission, createResponse, getFriendlyErrorMessage } from '../shared/utils';
import { getAuthenticatedUserFromEvent } from '../shared/auth';
import { getPrismaClient } from '../shared/db';
import { exMemberWelcomeEmailCommand, welcomeEmailCommand } from '../shared/email';
...
const registerManySchema = z.object({
  ids: z.array(z.string().uuid("ID inválido.")).min(1, "Pelo menos um ID de pedido é necessário."),
});
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    try {
        ...
        if (!validation.success) {
            return createResponse(400, { message: "Dados inválidos.", errors: validation.error.formErrors.fieldErrors });
        }
        ...
        for (const req of requestsToProcess) {
            try {
                ...
                const cognitoUser = await cognitoClient.send(
                    new AdminCreateUserCommand({
                        ...
                        UserAttributes: [
                            ...
                            { Name: "birthdate", Value: req.birthDate.toISOString().split("T")[0] },
                            { Name: "phone_number", Value: `+55${req.phone.replace(/\D/g, "")}` },
                            { Name: "custom:role", Value: req.roles.length > 0 ? req.roles[0].name : "" },
                            { Name: "custom:isExMember", Value: String(req.isExMember ?? false) },
                ...
                if (!cognitoUserId) throw new Error("Não foi possível obter o ID do usuário do Cognito.");

                const rolesToConnect = req.roles?.filter((role: { name: string; }) => role.name !== "Outro").map((role: { id: any; }) => ({ id: role.id })) || [];
                ...
                const newUser = await prisma.user.create({
                    data: {
                        ...
                        ...(rolesToConnect.length > 0 && { currentRoleId: rolesToConnect[0].id }),
                ...
                if (newUser.isExMember) {
                    await sesClient.send(exMemberWelcomeEmailCommand({ email: newUser.email, name: newUser.name }));
                } else {
                    await sesClient.send(welcomeEmailCommand({ email: newUser.email, name: newUser.name }));
                }
                ...
                results.failed.push({ id: req.id, email: req.email, reason: friendlyMessage });
                
                await prisma.registrationRequest.update({
                    where: { id: req.id },
                    data: { status: "REJECTED", adminNotes: `Falha na aprovação: ${friendlyMessage}` },
                });
            }
        }
        ...
        return createResponse(500, { message: "Ocorreu um erro interno no servidor.", error: error.message });
    }
};