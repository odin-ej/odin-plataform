...
async function executeSql(sql: string, parameters: any[] = []) {
  if (!dbClusterArn || !dbSecretArn) {
    throw new Error(
      "DB_CLUSTER_ARN ou DB_SECRET_ARN não configurados para RDS Data API."
    );
  }
...
async function getAuthenticatedUserAdapted(
  requestHeaders: any
  ...
  const authHeader =
    requestHeaders["authorization"] || requestHeaders["Authorization"]; // Headers podem ser case-insensitive
...
async function checkUserPermissionAdapted(
  userId: string,
  permission: string
): Promise<boolean> {
...
async function getConversationHistoryAdapted(
  conversationId: string
): Promise<Content[]> {
  // Alterado para Promise<Content[]>
...
const getModelForPrompt = (prompt: string, forcePro = false) => {
  ...
  const isSimple = simpleKeywords.some(
    (
      kw: string // Adicionado tipo explícito para kw
...
const shouldUseTools = (prompt: string) => {
  ...
  return toolKeywords.some((kw: string) =>
    normalized.includes(normalizeString(kw))
  ); // Adicionado tipo explícito para kw
};
...
export async function handler(
  ...
  try {
    ...
    const isDirector = await checkUserPermissionAdapted(
      user.id.stringValue,
      DIRECTORS_ONLY as string
    ...
    const userLastMessageDate = user.lastMessageDate?.stringValue
      ? new Date(user.lastMessageDate.stringValue)
      : null; // Acessar o valor da string
    ...
    const context = contextChunksRecords
      .map((c: any) => c.content.stringValue)
    ...
    if (updatedTitle)
      updateConversationParams.push({
        name: "title",
        value: { stringValue: updatedTitle },
      });
