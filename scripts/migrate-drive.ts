  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if ((error.code === "ECONNRESET" || error.code === 403) && attempt < maxRetries) {
        ...
        console.warn(
          `  - Tentativa ${attempt} falhou para "${driveItem.name}" (Erro: ${error.code}). Tentando novamente em ${delay / 1000}s...`
        );
...
async function processFiles(
  ...
  const filesToSync = driveItems.filter((item) => {
    ...
    return (
      !localFile ||
      new Date(localFile.googleDriveModifiedTime!) < driveModifiedTime
    );
  });
  ...
  for (const driveItem of filesToSync) {
    try {
      ...
      if (driveItem.mimeType === "application/vnd.google-apps.spreadsheet")
        fileName += ".xlsx";
      else if (driveItem.mimeType === "application/vnd.google-apps.document")
        fileName += ".docx";
      ...
      const localParentId = parentDriveId
        ? driveToLocalFolderMap.get(parentDriveId)
        : null;
      ...
      console.error(
        `  - ERRO CRÍTICO ao processar "${driveItem.name}". Pulando. Erro: ${error.message}`
      );
    }
  }
...
async function syncAllItems(
  driveItems: DriveItem[],
  localFiles: OraculoFile[],
  localFolders: OraculoFolder[],
  ownerId: string
) {
  console.log("\nETAPA 2: Preparando dados das pastas...");
  const folderUpserts = await processFolders(driveItems, ownerId);

  console.log("\nETAPA 3: Executando transação de criação de pastas...");
  await prisma.$transaction(
    folderUpserts.map((folderOp) => prisma.oraculoFolder.upsert(folderOp))
  );
  console.log("  - Criação de pastas concluída.");

  console.log("\nETAPA 4: Executando transação de atualização de hierarquia...");
  const allLocalFolders = await prisma.oraculoFolder.findMany({
    where: { googleDriveFolderId: { not: null } },
  });
  const driveIdToLocalIdMap = new Map(
    allLocalFolders.map((f) => [f.googleDriveFolderId!, f.id])
  );
  const driveFolders = driveItems.filter(
    (it) => it.mimeType === "application/vnd.google-apps.folder"
  );
  ...
  await prisma.$transaction(
    driveFolders.flatMap((driveFolder) => {
      const parentDriveId = driveFolder.parents?.[0];
      if (!parentDriveId) return [];
      const parentLocalId = driveIdToLocalIdMap.get(parentDriveId);
  ...
  console.log("  - Atualização de hierarquia concluída.");

  const finalLocalFolders = await prisma.oraculoFolder.findMany({
    where: { googleDriveFolderId: { not: null } },
  });
  const driveToLocalFolderMap = new Map(
    finalLocalFolders.map((f) => [f.googleDriveFolderId!, f.id])
  );

  console.log("\nETAPA 5: Processando arquivos (Downloads e Uploads)...");
  const fileUpserts = await processFiles(
    driveItems,
    localFiles,
    driveToLocalFolderMap,
    ownerId
  );

  console.log("\nETAPA 6: Executando transação dos arquivos...");
  if (fileUpserts.length > 0) {
    await prisma.$transaction(
      fileUpserts.map((fileOp) => prisma.oraculoFile.upsert(fileOp))
    );
...
async function main() {
  ...
  try {
    ...
    if (!ownerId)
      throw new Error("O 'ownerId' não foi definido. Por favor, edite o script.");
    ...
    console.log(
      `  - ${localFiles.length} arquivos e ${localFolders.length} pastas locais encontradas.`
    );
